...
class BaseSoC(SoCCore):
	def __init__(self):
		...
		#UART WIFI
		#SoCCore.add_csr(self,"uart_wifi")
		self.submodules.uart_wifi_phy = uart.UARTPHY(
			pads  = platform.request("uart_wifi"),
			clk_freq=self.sys_clk_freq,
			baudrate=9600)
		self.submodules.uart_wifi = ResetInserter()(uart.UART(self.uart_wifi_phy,
                                                    tx_fifo_depth=16,
                                                    rx_fifo_depth=16))
		self.csr.add("uart_wifi_phy", use_loc_if_exists=True)
		self.csr.add("uart_wifi", use_loc_if_exists=True)
		if hasattr(self.cpu, "interrupt"):
			self.irq.add("uart_wifi", use_loc_if_exists=True)
		else:
