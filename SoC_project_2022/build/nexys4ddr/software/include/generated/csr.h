...
/* uart_wifi */
#define CSR_UART_WIFI_BASE (CSR_BASE + 0x3800L)
#define CSR_UART_WIFI_RXTX_ADDR (CSR_BASE + 0x3800L)
#define CSR_UART_WIFI_RXTX_SIZE 1
static inline uint32_t uart_wifi_rxtx_read(void) {
	return csr_read_simple(CSR_BASE + 0x3800L);
}
static inline void uart_wifi_rxtx_write(uint32_t v) {
	csr_write_simple(v, CSR_BASE + 0x3800L);
}
#define CSR_UART_WIFI_TXFULL_ADDR (CSR_BASE + 0x3804L)
#define CSR_UART_WIFI_TXFULL_SIZE 1
static inline uint32_t uart_wifi_txfull_read(void) {
	return csr_read_simple(CSR_BASE + 0x3804L);
}
#define CSR_UART_WIFI_RXEMPTY_ADDR (CSR_BASE + 0x3808L)
#define CSR_UART_WIFI_RXEMPTY_SIZE 1
static inline uint32_t uart_wifi_rxempty_read(void) {
	return csr_read_simple(CSR_BASE + 0x3808L);
}
#define CSR_UART_WIFI_EV_STATUS_ADDR (CSR_BASE + 0x380cL)
#define CSR_UART_WIFI_EV_STATUS_SIZE 1
static inline uint32_t uart_wifi_ev_status_read(void) {
	return csr_read_simple(CSR_BASE + 0x380cL);
}
#define CSR_UART_WIFI_EV_STATUS_TX_OFFSET 0
#define CSR_UART_WIFI_EV_STATUS_TX_SIZE 1
static inline uint32_t uart_wifi_ev_status_tx_extract(uint32_t oldword) {
	uint32_t mask = ((1 << 1)-1);
	return ( (oldword >> 0) & mask );
}
static inline uint32_t uart_wifi_ev_status_tx_read(void) {
	uint32_t word = uart_wifi_ev_status_read();
	return uart_wifi_ev_status_tx_extract(word);
}
#define CSR_UART_WIFI_EV_STATUS_RX_OFFSET 1
#define CSR_UART_WIFI_EV_STATUS_RX_SIZE 1
static inline uint32_t uart_wifi_ev_status_rx_extract(uint32_t oldword) {
	uint32_t mask = ((1 << 1)-1);
	return ( (oldword >> 1) & mask );
}
static inline uint32_t uart_wifi_ev_status_rx_read(void) {
	uint32_t word = uart_wifi_ev_status_read();
	return uart_wifi_ev_status_rx_extract(word);
}
#define CSR_UART_WIFI_EV_PENDING_ADDR (CSR_BASE + 0x3810L)
#define CSR_UART_WIFI_EV_PENDING_SIZE 1
static inline uint32_t uart_wifi_ev_pending_read(void) {
	return csr_read_simple(CSR_BASE + 0x3810L);
}
static inline void uart_wifi_ev_pending_write(uint32_t v) {
	csr_write_simple(v, CSR_BASE + 0x3810L);
}
#define CSR_UART_WIFI_EV_PENDING_TX_OFFSET 0
#define CSR_UART_WIFI_EV_PENDING_TX_SIZE 1
static inline uint32_t uart_wifi_ev_pending_tx_extract(uint32_t oldword) {
	uint32_t mask = ((1 << 1)-1);
	return ( (oldword >> 0) & mask );
}
static inline uint32_t uart_wifi_ev_pending_tx_read(void) {
	uint32_t word = uart_wifi_ev_pending_read();
	return uart_wifi_ev_pending_tx_extract(word);
}
static inline uint32_t uart_wifi_ev_pending_tx_replace(uint32_t oldword, uint32_t plain_value) {
	uint32_t mask = ((1 << 1)-1);
	return (oldword & (~(mask << 0))) | (mask & plain_value)<< 0 ;
}
static inline void uart_wifi_ev_pending_tx_write(uint32_t plain_value) {
	uint32_t oldword = uart_wifi_ev_pending_read();
	uint32_t newword = uart_wifi_ev_pending_tx_replace(oldword, plain_value);
	uart_wifi_ev_pending_write(newword);
}
#define CSR_UART_WIFI_EV_PENDING_RX_OFFSET 1
#define CSR_UART_WIFI_EV_PENDING_RX_SIZE 1
static inline uint32_t uart_wifi_ev_pending_rx_extract(uint32_t oldword) {
	uint32_t mask = ((1 << 1)-1);
	return ( (oldword >> 1) & mask );
}
static inline uint32_t uart_wifi_ev_pending_rx_read(void) {
	uint32_t word = uart_wifi_ev_pending_read();
	return uart_wifi_ev_pending_rx_extract(word);
}
static inline uint32_t uart_wifi_ev_pending_rx_replace(uint32_t oldword, uint32_t plain_value) {
	uint32_t mask = ((1 << 1)-1);
	return (oldword & (~(mask << 1))) | (mask & plain_value)<< 1 ;
}
static inline void uart_wifi_ev_pending_rx_write(uint32_t plain_value) {
	uint32_t oldword = uart_wifi_ev_pending_read();
	uint32_t newword = uart_wifi_ev_pending_rx_replace(oldword, plain_value);
	uart_wifi_ev_pending_write(newword);
}
#define CSR_UART_WIFI_EV_ENABLE_ADDR (CSR_BASE + 0x3814L)
#define CSR_UART_WIFI_EV_ENABLE_SIZE 1
static inline uint32_t uart_wifi_ev_enable_read(void) {
	return csr_read_simple(CSR_BASE + 0x3814L);
}
static inline void uart_wifi_ev_enable_write(uint32_t v) {
	csr_write_simple(v, CSR_BASE + 0x3814L);
}
#define CSR_UART_WIFI_EV_ENABLE_TX_OFFSET 0
#define CSR_UART_WIFI_EV_ENABLE_TX_SIZE 1
static inline uint32_t uart_wifi_ev_enable_tx_extract(uint32_t oldword) {
	uint32_t mask = ((1 << 1)-1);
	return ( (oldword >> 0) & mask );
}
static inline uint32_t uart_wifi_ev_enable_tx_read(void) {
	uint32_t word = uart_wifi_ev_enable_read();
	return uart_wifi_ev_enable_tx_extract(word);
}
static inline uint32_t uart_wifi_ev_enable_tx_replace(uint32_t oldword, uint32_t plain_value) {
	uint32_t mask = ((1 << 1)-1);
	return (oldword & (~(mask << 0))) | (mask & plain_value)<< 0 ;
}
static inline void uart_wifi_ev_enable_tx_write(uint32_t plain_value) {
	uint32_t oldword = uart_wifi_ev_enable_read();
	uint32_t newword = uart_wifi_ev_enable_tx_replace(oldword, plain_value);
	uart_wifi_ev_enable_write(newword);
}
#define CSR_UART_WIFI_EV_ENABLE_RX_OFFSET 1
#define CSR_UART_WIFI_EV_ENABLE_RX_SIZE 1
static inline uint32_t uart_wifi_ev_enable_rx_extract(uint32_t oldword) {
	uint32_t mask = ((1 << 1)-1);
	return ( (oldword >> 1) & mask );
}
static inline uint32_t uart_wifi_ev_enable_rx_read(void) {
	uint32_t word = uart_wifi_ev_enable_read();
	return uart_wifi_ev_enable_rx_extract(word);
}
static inline uint32_t uart_wifi_ev_enable_rx_replace(uint32_t oldword, uint32_t plain_value) {
	uint32_t mask = ((1 << 1)-1);
	return (oldword & (~(mask << 1))) | (mask & plain_value)<< 1 ;
}
static inline void uart_wifi_ev_enable_rx_write(uint32_t plain_value) {
	uint32_t oldword = uart_wifi_ev_enable_read();
	uint32_t newword = uart_wifi_ev_enable_rx_replace(oldword, plain_value);
	uart_wifi_ev_enable_write(newword);
}
#define CSR_UART_WIFI_TXEMPTY_ADDR (CSR_BASE + 0x3818L)
#define CSR_UART_WIFI_TXEMPTY_SIZE 1
static inline uint32_t uart_wifi_txempty_read(void) {
	return csr_read_simple(CSR_BASE + 0x3818L);
}
#define CSR_UART_WIFI_RXFULL_ADDR (CSR_BASE + 0x381cL)
#define CSR_UART_WIFI_RXFULL_SIZE 1
static inline uint32_t uart_wifi_rxfull_read(void) {
	return csr_read_simple(CSR_BASE + 0x381cL);
}

/* servo_driver */
#define CSR_SERVO_DRIVER_BASE (CSR_BASE + 0x4000L)
#define CSR_SERVO_DRIVER_RST_ADDR (CSR_BASE + 0x4000L)
#define CSR_SERVO_DRIVER_RST_SIZE 1
static inline uint32_t servo_driver_rst_read(void) {
	return csr_read_simple(CSR_BASE + 0x4000L);
}
static inline void servo_driver_rst_write(uint32_t v) {
	csr_write_simple(v, CSR_BASE + 0x4000L);
}
#define CSR_SERVO_DRIVER_POS_ADDR (CSR_BASE + 0x4004L)
...
/* i2c_master */
#define CSR_I2C_MASTER_BASE (CSR_BASE + 0x4800L)
#define CSR_I2C_MASTER_W_ADDR (CSR_BASE + 0x4800L)
...
#define CSR_I2C_MASTER_R_ADDR (CSR_BASE + 0x4804L)
